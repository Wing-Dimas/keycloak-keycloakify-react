            ...
            {formFieldStates.map(({ attribute, displayableErrors, valueOrValues }) => {
                return (
                    <Fragment key={attribute.name}>
                        ...
                        <div
                            className={clsx(kcClsx("kcFormGroupClass"), "mb-2")}
                            style={{
                                display: attribute.name === "password-confirm" && !doMakeUserConfirmPassword ? "none" : undefined
                            }}
                        >
                            {attribute.name !== "lastName" && (
                                <div className={clsx(kcClsx("kcLabelWrapperClass"), "mb-2")}>
                                    <label
    ...
    switch (attribute.annotations.inputType) {
        ...
        default: {
            ...
            if (attribute.name === "lastName") {
                return <input type="hidden" value={valueOrValues} name={attribute.name} id={attribute.name} />;
            }
            ...
            if (attribute.name === "firstName") {
                ...
                useEffect(() => {
                    ...
                    if (name.length === 1) {
                        // Jika hanya ada satu kata, firstname dan lastname diambil dari kata pertama
                        firstname = name[0] as string;
                        lastname = name[0] as string;
                    } else {
                        // Jika lebih dari satu kata, kata terakhir sebagai lastname, sisanya sebagai firstname
                        lastname = name.pop() as string; // Ambil kata terakhir
                        firstname = name.join(" ") as string; // Gabungkan sisanya sebagai firstname
                    }
                    props.dispatchFormAction({
                        action: "update",
                        name: "firstName",
                        valueOrValues: firstname
                    });
                    props.dispatchFormAction({
                        action: "update",
                        name: "lastName",
                        valueOrValues: lastname
                    });
                }, [fullname]);
                const fullNameProps = {
                    ...props,
                    ...
                    valueOrValues: fullname
                };

                return (
                    <>
                        <input type="hidden" value={valueOrValues} name={attribute.name} id={attribute.name} />
                        <InputTag
                            {...fullNameProps}
                            fieldIndex={undefined}
                            valueOrValues={fullname}
    ...
    return (
        <div className={clsx(kcClsx("kcInputGroup"), "relative")}>
            ...
            >
                ...
                >
                    <path className={`${!isPasswordRevealed && "hidden"}`} d="M9.88 9.88a3 3 0 1 0 4.24 4.24"></path>
                    <path
                        className={`${!isPasswordRevealed && "hidden"}`}
                    ...
                    ></path>
                    <line className={`${!isPasswordRevealed ? "block" : "hidden"}`} x1="2" x2="22" y1="2" y2="22"></line>
                    <path className={`block`} d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"></path>
                    <circle className={`${isPasswordRevealed ? "hidden" : "block"}`} cx="12" cy="12" r="3"></circle>
                </svg>
            </button>
        </div>
    );
    ...
    return (
        <>
            <input
                ...
                className={clsx(
                    kcClsx("kcInputClass"),
                    "bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                )}
    ...
    const { classDiv, classInput, classLabel, inputType } = (() => {
        ...
        switch (inputType) {
            ...
            case "multiselect-checkboxes":
                return {
                    inputType: "checkbox",
                    classDiv: clsx(kcClsx("kcInputClassCheckbox"), "flex items-center gap-2"),
            ...
            {options.map(option => (
                <div key={option} className={classDiv}>
                    ...
                    <label
                        ...
                        className={clsx(
                            `${classLabel}${attribute.readOnly ? ` ${kcClsx("kcInputClassRadioCheckboxLabelDisabled")}` : ""}`,
                            "text-sm font-medium text-primary-600 hover:underline dark:text-primary-500 cursor-pointer"
                        )}
                    >
